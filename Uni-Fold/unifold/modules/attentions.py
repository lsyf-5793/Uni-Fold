...
class MSAAttention(nn.Module):
    ...
    def _attn_forward(self, m, mask, bias: Optional[torch.Tensor] = None):
        ...
        if self.mha.linear_g is not None:
            # gating, use raw query input
            g = self.mha.linear_g(x)
        
        q = self.mha.linear_q(x)
        q *= self.mha.norm
        k = self.mha.linear_k(x)
        v = self.mha.linear_v(x)
        q = q.view(q.shape[:-1] + (self.mha.num_heads, -1)).contiguous()
        k = k.view(k.shape[:-1] + (self.mha.num_heads, -1)).contiguous()
        v = v.view(v.shape[:-1] + (self.mha.num_heads, -1)) 
        x = DS4Sci_EvoformerAttention(Q=q,K=k,V=v,biases=[mask,bias])
        ...
        # merge heads
        x = nn.functional.linear(x, self.mha.linear_o.weight)
        return x
...
class TriangleAttention(nn.Module):
    ...
    def forward(
        ...
        if chunk_size is not None:
            ...
            x = DS4Sci_EvoformerAttention(Q=q,K=k,V=v,biases=[attn_mask,triangle_bias])
